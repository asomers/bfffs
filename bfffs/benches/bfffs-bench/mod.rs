...
async fn bench_all<'a>(geom_regex: &'a Option<Regex>, harness: &'a Harness) {
    for benchmark in [
        BenchmarkImpl::FsCreate(fs_create::Cli::parse_from::<_, OsString>([]).build()),
        BenchmarkImpl::FsDestroy(fs_destroy::Cli::parse_from::<_, OsString>([]).build())
    ] {
...
async fn main() {
    ...
    let benchmark = match cli.subcommand {
        Some(SubCommand::FsCreate(subcmd)) => BenchmarkImpl::FsCreate(subcmd.build()),
        Some(SubCommand::FsDestroy(subcmd)) => BenchmarkImpl::FsDestroy(subcmd.build()),
        None => {
